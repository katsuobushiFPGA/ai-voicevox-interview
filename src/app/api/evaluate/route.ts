...
function buildEvaluationPrompt(answers: Answer[]): string {
  ...
  return `あなたはエンジニア中途採用面接の評価者です。以下の面接の質問と回答を評価してください。
...
★評価基準（以下の5つの観点で総合的に採点してください）:

①結論ファースト
- 結論から話せているか？理由や背景から入ってしまい、質問とズレた回答になっていないか。
- 結論→理由→具体例の順で論理的に構成されているか。

②適切な回答量（キャッチボール）
- 1つの質問に対して結論＋理由背景で2〜3文程度で答えられているか。
- 長すぎず短すぎず、面接のキャッチボールとして適切な分量か。

③ポジティブ変換
- 「〜ない」「〜できない」というネガティブな表現を使わず、「〜したい」「〜を目指している」と前向きに変換できているか。
- 転職理由を過去形（後ろ向き）ではなく未来形（前向き）で話せているか。
- OK例：「今まで以上にスキルアップしたいと考えたから」
- NG例：「現職だとスキルアップできないから」

④回答の一貫性
- 転職理由・志望理由・キャリアプラン・転職の軸に一貫性があるか。
- 例えば、転職理由でスキルアップと言いながら企業選びの軸でWLBを重視するなどのちぐはぐさがないか。

⑤技術・キャリア志向
- 条件面（年収、WLB等）ではなく、技術やキャリアで叶えたいことを中心に語れているか。
- 向上心や技術で何を成し遂げたいかが伝わるか。
- 具体的な技術名や経験を挙げて説得力がある回答になっているか。

追加の採点ポイント:
- 無回答や極端に短い回答は低得点
- 成功体験では、本当に自身の力である根拠と再現性を語れているか
- 失敗体験では、原因分析と改善への取り組み・結果まで語れているか
- 強みは具体的なエピソード付きで語れているか
- キャリアプランは時間軸（3年後・5年後・10年後）を意識した回答か`;
}
